...
Write-Output $WebHookData.RequestBody
    ...
    if ($WebHookData.PSObject.Properties.Name -contains 'RequestBody') {
        if ($WebHookData.RequestBody.GetType().Name  -ieq 'String') {
            try {
                $RequestBodyObj = $WebHookData.RequestBody | ConvertFrom-Json
        ...
        Write-Output ("   # ServiceName:`t`t[{0}]" -f $RequestBody.ServiceName)
        ...
        $arguments = "-serviceName $($RequestBody.ServiceName)"
